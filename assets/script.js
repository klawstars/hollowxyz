...
document.addEventListener('alpine:init', () => {
  Alpine.data('app', () => ({
    ...
    appCustomer: {
      ...
      getTokenCookieOptions(expiresDays = null) {
        const options = {
          path: '/',
          sameSite: 'Strict',
        ...
        if (typeof expiresDays === 'number' && Number.isFinite(expiresDays)) {
          options.expires = expiresDays;
        }
      ...
      async modalLogin() {
        ...
        try {
          ...
          console.error(error);
          this.modalOtpError = 'Network error. Please try again.';
        } finally {
          this.modalLoading = false;
        }
      },
      ...
      async logout() {
        ...
        Cookies.remove('shop_customer_token', this.getTokenCookieOptions());
    ...
    appMaintenance: {
      ...
      modalClose() {
        $('#maintenance-login-modal').modal('hide');
